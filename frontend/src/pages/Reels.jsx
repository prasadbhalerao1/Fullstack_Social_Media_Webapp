...
const Reels = () => {
  ...
  return (
    <div className="bg-black/95 flex text-white min-h-screen">
      <Sidebar />
      <main className="flex-1 w-full max-w-2xl mx-auto flex flex-col items-center overflow-hidden font-sans select-none h-screen pb-safe">
        {/* Loading State */}
        {loading && reels.length === 0 ? (
          ...
          /* Vertical Stack of Reel Cards with Snap Scrolling */
          <div className="w-full h-[calc(100dvh-4.5rem)] md:h-[calc(100vh-2rem)] overflow-y-scroll snap-y snap-mandatory scroll-smooth no-scrollbar flex flex-col items-center gap-0">
            ...
            {!hasMore && reels.length > 0 && (
              <div className="py-6 text-center text-neutral-600 text-xs shrink-0">
...
const ReelCard = ({
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-[calc(100dvh-4.5rem)] md:h-[calc(100vh-2rem)] flex items-center justify-center snap-start snap-always py-2 px-1 shrink-0"
    >
      <div className="h-full w-auto max-w-full aspect-9/16 relative bg-neutral-950 rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col justify-center shadow-2xl border border-white/5 group">
        <video
          ref={videoRef}
          src={reel.mediaUrl}
